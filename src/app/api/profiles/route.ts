...
async function getHandler(req: NextRequest) {
  ...
  if (!key && !discordId) return NextResponse.json({ error: 'uid or discordId required' }, { status: 400 })
  ...
  // Resolve to internal user id from either numeric uid, email, discordId, or direct userId
  let user = null as any
  
  // Try Discord ID first (if provided)
  if (discordId) {
    user = await prisma.user.findFirst({ where: { discordId } })
  }
  
  // Try numeric uid
  if (!user && key && /^\d+$/.test(key)) {
    user = await prisma.user.findFirst({ where: { uid: Number(key) } })
  }
  // Try email
  if (!user && key && key.includes('@')) {
