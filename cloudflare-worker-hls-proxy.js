...
export default {
  async fetch(request, env, ctx) {
    ...
    if (isM3U8Request || contentType?.includes('application/vnd.apple.mpegurl')) {
      ...
      const rewritten = playlist
        ...
        .map((line, index) => {
          ...
          try {
            ...
            // For other segments (like MegaUp), return original
            return trimmed;
          } catch (error) {
      ...
      // Log a sample of the rewritten playlist (first 20 lines) for debugging
      const sampleLines = rewritten.split(/\r?\n/).slice(0, 20);
      console.log('📋 Sample of rewritten playlist (first 20 lines):');
      sampleLines.forEach((line, idx) => {
        if (line.trim() && !line.trim().startsWith('#')) {
          console.log(`  Line ${idx + 1}: ${line.substring(0, 200)}`);
        } else if (line.includes('EXT-X-KEY') || line.includes('URI=')) {
          console.log(`  Line ${idx + 1}: ${line.substring(0, 200)}`);
        }
      });
